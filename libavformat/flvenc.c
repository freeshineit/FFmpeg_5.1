...
static void flv_write_codec_header(AVFormatContext *s, AVCodecParameters *par, int64_t ts)
{
    ...
    if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4 || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_OPUS || par->codec_id == AV_CODEC_ID_VP8 || par->codec_id == AV_CODEC_ID_VP9)
    {
        int64_t pos;
        avio_w8(pb,
                par->codec_type == AVMEDIA_TYPE_VIDEO ? FLV_TAG_TYPE_VIDEO : FLV_TAG_TYPE_AUDIO);
        ...
        if (par->codec_id == AV_CODEC_ID_AAC)
        {
            ...
            if (!par->extradata_size && (flv->flags & FLV_AAC_SEQ_HEADER_DETECT))
            {
                ...
                int channels = flv->audio_par->ch_layout.nb_channels - (flv->audio_par->ch_layout.nb_channels == 8 ? 1 : 0);
                uint8_t data[2];
...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4 || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_VP8 || par->codec_id == AV_CODEC_ID_VP9 || par->codec_id == AV_CODEC_ID_OPUS)
    {
        size_t side_size;
    ...
    if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4 || par->codec_id == AV_CODEC_ID_H265 || par->codec_id == AV_CODEC_ID_VP8 || par->codec_id == AV_CODEC_ID_VP9)
    {
    ...
    else
    {
        ...
        else if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4 || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_VP8 || par->codec_id == AV_CODEC_ID_VP9)
        {
